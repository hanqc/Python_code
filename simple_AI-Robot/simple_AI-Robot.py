...
def get_response(_info):
	...
	if len(response_dic['results']) == 1:
		return response_dic['results'][0]['values']['text']
	else:
		return response_dic['results'][1]['values']['text']+response_dic['results'][0]['values']['url']

@itchat.msg_register(itchat.content.TEXT, isGroupChat=True)#isGroupChat为false表示忽略群聊
def text_reply(msg):
	global flag
	if msg["Text"]=="闭嘴":
		flag = 1
